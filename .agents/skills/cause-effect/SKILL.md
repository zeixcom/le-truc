...
> Written against **@zeix/cause-effect 1.5.1**. Verify against the installed `node_modules/@zeix/cause-effect/README.md` and `src/` on major upgrades. If the README and `src/` disagree, **`src/` wins**.
...
| Derived keyed list | `deriveList(list, fn)` | Sync or async per-item mapping; chains for pipelines. Replaces `list.deriveCollection(fn)` |
