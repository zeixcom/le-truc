...
Le Truc docs build pipeline and dev server architecture. Read before touching any major component.
...
`docsMarkdown` is a multi-stage derived pipeline:
...
GitHub Pages cannot set response headers dynamically, so `Accept: text/markdown` negotiation is unavailable in production. The build pipeline uses **parallel path discovery** instead:
...
Every HTML page has a parallel `*.md` file in `docs/` generated by `mdMirrorEffect`. The dev server serves these as static files at the same path with a `.md` extension — no special route handling is needed.
