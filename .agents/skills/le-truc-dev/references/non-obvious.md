...
**Granularity follows the phase.** A throw from the factory or an extension's `onConnect` costs the whole component — a factory is one indivisible consumer function and cannot be resumed past a throw. A throw from a single effect descriptor costs only that descriptor: `activateResult()` takes an optional `onError` and, when given one, contains each thunk separately so the siblings still activate. The host passes a handler; `each()`'s and `reconcile()`'s nested `activateResult` calls do not, because they already run *inside* a descriptor the host's own containment wraps.
