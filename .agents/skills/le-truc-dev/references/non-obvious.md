...
`activateDescriptors()` (`src/helpers/reactive.ts`) discards the return value of every descriptor it calls during activation. `watch()`/`on()`/`pass()` are unaffected because they call `createEffect()`/`createScope()` *inside* their own descriptor body. That self-registers cleanup onto the active owner, regardless of what the outer caller does with the return value.

A raw hand-authored descriptor — `() => { setup(); return cleanup }`, with no internal `createEffect`/`createScope` call — has **no such registration**: called bare, its cleanup is silently dropped and never runs on disconnect. This was a real, previously-shipping bug in several example components. The fix wraps the raw descriptor in `watch(() => true, descriptor)`: `() => true` has no signal dependency so it runs once, and `watch()`'s internal `createEffect()` call self-registers the descriptor's returned cleanup. This is the only registration path for a hand-authored descriptor (since v3.0 the factory's return value is ignored), so always register them this way.
...
**The property-key (`'value'`) and bare-writable-signal short forms were removed in v3.0 (ADR 0012).** A thunk (`() => host.prop`, read-only) or descriptor (`{ get, set }`, mediated writable) is the only accepted input — at the types and at runtime, including bare read-only signals (`Memo`/`Task`/`Sensor`), which 2.x admitted without a warning. A retired form resolves to nothing and fails the eager validation below, swapping nothing.

**Every entry in `props` is validated before any signal is swapped.** If a passed prop doesn't exist on the target, can't be resolved to a signal, or isn't Slot-backed — which is exactly what happens when the target is a non-Le-Truc element, or the prop is read-only/computed — `pass()` throws `InvalidPassPropertyError` naming every failing prop, instead of silently no-op'ing. The commit is atomic, so a failure leaves the target exactly as it was. This is a deferred-activation throw (ADR 0007): it happens inside `connectedCallback`, after the calling factory has already returned, so the factory's own code cannot catch it — it is contained per descriptor and reported instead ([ADR 0028](../../../../adr/0028-tiered-error-surfacing.md), which supersedes ADR 0011). `LTC012` decides the same question at compile time for a registry-known target, so the runtime check is the backstop for hand-authored and foreign elements.

## Connect-Time Errors Are Contained — All Of Them

`connectedCallback` catches every throw that reaches it ([ADR 0028](../../../../adr/0028-tiered-error-surfacing.md)). There is no exception list, no brand, nothing to remember. `TrucError`/`TrucTypeError`/`isContractError()` existed briefly and are gone; ADR 0011's "surfaces as an uncaught `pageerror`" clause is retired with it.

**Granularity follows the phase.** A throw from the factory or an extension's `onConnect` costs the whole component — a factory is one indivisible consumer function and cannot be resumed past a throw. A throw from a single effect descriptor costs only that descriptor: `activateDescriptors()` takes an optional `onError` and, when given one, contains each thunk separately so the siblings still activate. The host passes a handler; `each()`'s and `reconcile()`'s nested `activateDescriptors` calls do not, because they already run *inside* a descriptor the host's own containment wraps.

**The two escaping paths escape structurally, not by exemption.** `defineComponent()` throws `InvalidComponentNameError`/`ExtensionCollisionError` before the class exists, and `dangerouslyBindInnerHTML` re-throws a Trusted Types violation from a `queueMicrotask`. Neither is inside `connectedCallback`, so neither needed a marker to get out — which is the point: deleting the brand cannot silently break them.

**Reporting.** Whole-component failures use `reportConnectFailure(host, phase, error)`, per-descriptor ones `reportEffectFailure(host, descriptor, error)`. The descriptor label comes from `describeDescriptor()` in `src/internal.ts`, backed by a `WeakMap` that `pushDescriptor()` fills with the helper name it already receives — a lookup rather than a property, so a hand-authored consumer function is never mutated (it gets a generic label instead). Each failure is reported once per instance: a failed factory sets `#connectFailed` and reconnects return immediately, and a failed descriptor is recorded in `#reportedFailures` so a reslot cycle does not re-report it.

**Writing a message.** A Tier 2 failure means the component degraded to markup that is already correct — never word it as a crash. Tech Writer owns the copy in `src/errors.ts` (see the tech-writer skill); write a first draft and hand it off.

A throw inside a `watch()` *handler* never reaches any of this: cause-effect's `match()` routes it to the `err` branch, defaulting to `console.error`. The containment sees hand-authored descriptors, `pass()` validation, and whatever the factory does directly.
