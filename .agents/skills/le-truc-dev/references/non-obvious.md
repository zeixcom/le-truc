...
As of v2.3 there is **no `DEV_MODE` const** in `util.ts`: every DEV-gated diagnostic is guarded inline by `process.env.DEV_MODE === 'true'` at its use site. This exact form is load-bearing: Bun's minifier does no cross-reference constant propagation (an imported `const DEV_MODE = false` — and even `var o = !1; if (o) …` — survives minification), but it does fold a same-type literal comparison in an `if` condition. The build therefore defines the **string** `'"false"'` (prod) or `'"true"'` (dev) — a bare boolean define produces `false === 'true'`, which Bun does *not* fold, silently shipping all DEV code again (this was a real ~450B-gzipped regression). In `&&` chains, keep the env check in first position. `test/regression-bundle.test.ts` asserts known DEV-only strings are absent from the prod bundle.

Since v2.4, `debug()` (`src/extensions/debug.ts`) adds a per-instance runtime `debug: boolean` property on top of this — but it is a narrowing of `DEV_MODE`, not a second mode: the property only exists when `DEV_MODE` is also on, and every effect it drives re-checks `process.env.DEV_MODE === 'true'` internally at fire time, the same foldable form as everything else in this section.

## `debug()` is a Deliberate Exception to "`component.ts` Never Imports a Concrete Feature Module"

ADR 0019 established that `component.ts` reaches every extension only through the array a caller passes — never a static `import` of a concrete extension module — so tree-shaking is a structural guarantee, not something DCE merely happens to achieve. `debug()` breaks this on purpose (ADR 0022): `component.ts` statically imports `debug()` from `src/extensions/debug.ts` and appends it to every component's merged extensions whenever `process.env.DEV_MODE === 'true'`, regardless of what the third parameter contains. This is required for the feature to work at all — "zero code changes to instrument any component, including ones the author didn't write" is only possible if `defineComponent()` itself injects it. **Do not "fix" this by making `debug()` opt-in or removing the static import** — that defeats the point. Production safety here rests on the same `process.env.DEV_MODE === 'true'` constant-fold + DCE pattern as the rest of this section, not on structural unreachability.
