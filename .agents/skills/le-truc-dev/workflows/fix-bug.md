...
- If it's a hand-authored `EffectDescriptor` (not produced by `watch`/`on`/`pass`/`each`/`provideContexts`): was it registered via `watch(() => true, descriptor)`? A bare-called descriptor with no internal `createEffect`/`createScope` call has its cleanup silently discarded by `activateDescriptors()` — see non-obvious.md's hand-authored-descriptor entry
