...
For effect bugs — trace through `makeWatch` / `makeOn` / `makePass` / `makeRun` in `src/helpers/reactive.ts` and `src/helpers/events.ts`:
...
- If it's a hand-authored `EffectDescriptor` (not produced by `watch`/`on`/`pass`/`each`/`provideContexts`): was it registered via `run()`? A bare `return`ed descriptor with no internal `createEffect`/`createScope` call has its cleanup silently discarded by `activateResult()` — see non-obvious.md's `run()` entry
