...
| `NoActiveCollectorError` | `watch()`/`on()`/`pass()`/`each()`/`provideContexts()` ran outside synchronous factory setup — usually deferred into a callback, or an `async` factory. | Call the helper directly in setup; move the deferred condition inside the effect. | **2**. Compiler: `TSRX045` (deferred call), `TSRX008` (`async` factory) |
...
| `TSRX034` | `disabled`/`checked` on a submittable form control has no server-renderable value, and no server phase resolves the expression in any tier, so the attribute is omitted — leaving the control enabled-and-submittable (or unchecked) regardless of author intent. | Trace the value to a server-known prop or signal, or give the element an explicit static default. | error |
