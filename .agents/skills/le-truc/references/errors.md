...
| `LTC033` | A **static** child, a server-rendered attribute, or a server-data loop's items read `Date`/`Intl`, a random-number generator (`Math.random()`, `crypto.randomUUID()`, `crypto.getRandomValues()`), or a locale method — the *build machine's* reading, baked into the page permanently. | Make it reactive, or take the value as a server arg. | error |
...
| `LTC054` | The partial-readiness invariant ([ADR 0034](../../../../adr/0034-distribution-tsx-only-compiler-package-and-template-emission.md) s4). Two forms. (1) A position the server evaluates at build time reads page context (`document`, `window`, `navigator`, `location`, storage, …), directly or through a setup helper: a static attribute or text child, a condition, a setup const, a compose arg, or a reactive thunk that the server folds. (2) The reserved `i18n` record is destructured for a member it does not carry, or with a rest element. Server-rendered HTML can depend only on the component's own args and the `i18n` record's `lang`, `t`, `timeZone`, `currency` and `dir`. | Pass the value in as an arg, or read it in the factory (`watch()` or an `on()` handler), where it runs in the browser. For form (2), destructure only the record's own members. The set is declared in `server/compiler/fold-inputs.ts`, and a change to it is an Architect decision. | error |
