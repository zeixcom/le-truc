...
| Layer | File | Fix when |
|---|---|---|
| SKILL.md | `.claude/skills/<name>/SKILL.md` | Intake options, routing, scope declaration, or workflow index are wrong |
| Reference | `.claude/skills/<name>/references/*.md` | A fact about an API, type, or behavior is incorrect or missing |
| Workflow | `.claude/skills/<name>/workflows/*.md` | A process step leads the agent to wrong output |
