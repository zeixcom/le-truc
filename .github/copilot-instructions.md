...
- Do not break the `customElements.define()` registration in `src/component.ts` — `makeElementQueries` in `src/helpers/dom.ts` relies on it for dependency detection.

## Conventions to enforce
- Component names must include a hyphen and match `/^[a-z][a-z0-9-]*$/`.
- Props initializers: plain values, `Signal`s, parsers from `src/parsers/*`, or callbacks — parsers run once at connect time via `getAttribute()`, not via `observedAttributes` (dropped entirely in v2.0).
