...
- **One tag, one authored source per surface — variants are folder-local**: a corpus folder may carry a **variant set** — the `.ts` twin, the `.tsrx` and the `.tsx` spelling, all declaring one canonical tag (ADR 0039). The build compiles every member, asserts their CSS byte-identical, and serves the selected surface's artifacts under the canonical names (`.tsx` by default; `variantSurface`/`variantOverrides` in `le-truc.config.json`). A tag two same-surface files declare — or that two folders declare — still fails the build naming both (LTC048). Within a variant set exactly one member declares the `declare global` `HTMLElementTagNameMap` entry (the `.ts` twin when present, else the `.tsrx`, else the `.tsx`); duplicate entries are a TS 2717 error. Migrate a tag to `.tsx` by **adding** the `.tsx` source alongside the `.tsrx` twin — it then becomes the served surface — not by replacing the twin.
