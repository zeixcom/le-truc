...
- **`expose()` on a built-in IDL property name is silently skipped**: `'lang' in this` (also `dir`, `title`, `hidden`, every global-attribute reflection) is always true for an element, so `#initSignals`'s `prop in this` guard skips the initializer before any accessor is installed — no error, no reactivity, and reads of `host.lang` hit the NATIVE accessor, i.e. the element's own attribute, live. The attribute is the only channel for these names: seed it, don't expose it (`basic-pluralize` materializes its walked locale onto the `lang` attribute at connect rather than exposing a prop; LT-191). A parser-applied seed (`asString()(getLocale(host))`) does not help — the guard runs before the initializer would.
