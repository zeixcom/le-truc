...
- **One tag, one authored source, whichever surface**: the corpus scan globs `.tsrx` AND `.tsx` into one registry, and a tag two files declare fails the build naming both, dropping both (LTC048). Migrate a tag to `.tsx` by replacing the `.tsrx` source in the same change, not alongside it.
