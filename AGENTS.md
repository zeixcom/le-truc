...
- **Dependency resolution races the child's own upgrade**: if a queried custom element isn't defined within `DEPENDENCY_TIMEOUT`, a `DependencyTimeoutError` is logged and the parent's effects activate anyway against a possibly-undefined child — intentional progressive enhancement. But a parent write to that child's property (e.g. `bindProperty`) lands as a plain instance property; on upgrade, `#initSignals` skips wrapping any prop already present on the instance ("explicit DOM value wins"), so the prop never becomes reactive and the child's own initializer is discarded. Don't write to a timed-out child's properties.
