...
- **Control flow**: `.tsx` spells conditions as expressions — ternaries and `&&`, `.map()` for loops, an IIFE for switch — and the error and async boundaries as the `truc:` intrinsic element `<truc:try pending={…} catch={e => …}>` ([ADR 0041](adr/0041-truc-intrinsic-elements-for-compiler-consumed-constructs.md)): all arms render and are `hidden`-toggled by which state won, `pending` being no-value-yet. A compiler-consumed construct gets a `truc:` element only when its JavaScript spelling misstates what it means, it needs no generic type parameter, and it joins the host profile's closed vocabulary (ADR 0041). There is no `stale` arm on either surface (owner withdrawal, LT-211 — the client toggles hidden/disabled on server-rendered arms and never re-renders their content); the in-flight state is the reactive `isPending(signal)` idiom beside the boundary, folded server-side and watched client-side. `.tsrx` keeps the directive grammar — `@if`/`@switch`/`@try`/`@for` with statement-context arms — which is the ergonomic edge the dual ruling retains; its boundary is the same three-arm shape.
- **CSS**: `.tsx` wraps the stylesheet in the `css` template tag — ``<style>{css`…`}</style>`` — so editors highlight it as CSS; a bare template literal is accepted. `.tsrx`'s `<style>` block is raw CSS. A compiled component's sheet is **shadow-root CSS** ([ADR 0033](adr/0033-scope-component-styles-by-custom-element-name.md); ships in 3.0 with LT-268/LT-304/LT-306): `:host` for the host, bare selectors for the internals. The compiler scopes it in light DOM so rules stop at every custom element the template renders: `@scope (my-element) to (<rendered custom-element tags> > *)` where the configured CSS target (`cssTargets`, default Baseline widely available) supports it, a zero-specificity `:where(:not(…))` lowering otherwise, and `:host` as `:where(:scope)` in both, so page styles win over host rules as they do over a shadow root's. A rule led by the component's own tag, `::slotted()` in light mode and `:host-context()` are tier 1 errors. The escape hatch is TSRX's `:global`, limited to whole rules (`:global(body.scroll-lock) { … }` or a top-level `:global { … }` block), which are emitted outside the scope. Scoping does not stop page CSS from reaching in; only a declarative shadow root does, and that opt-in is unbuilt. Hand-written `.ts` twins keep their verbatim, tag-led `.css`. Hash classes are declined. Until LT-304 lands, styles emit verbatim and dedented. Checks over the parsed sheet are recorded in [ADR 0042](adr/0042-the-component-stylesheet-as-a-compiler-artifact.md), and style composition stays on the [roadmap](ROADMAP.md).
- **Dynamic tags**: `.tsrx`'s `<{expr}>` is rejected (LTC053, tier 1, LT-213), and `.tsx` has no spelling for it. A tag chosen from a server arg is spelled as a conditional over static elements. The recorded design admits only a server-known expression that names an HTML element (never a custom element, so composition stays PascalCase-only and statically resolvable), folded in the Folded tier, and spelled `<truc:element tag={…}>` in `.tsx`. It is built when a migration needs it.
- **Shorthand**: the `{count}` attribute shorthand is `.tsrx`-only; `.tsx` spells `count={count}`, lowering to the same IR.
- **Type checking**: authored `.tsx` is checked directly by `tsc` against the ambient host profile — a strict per-element light-DOM attribute table where `class`/`for` have no React aliases, a composed child declares its pass surface on its own args type, the boundary arms are typed `JSX.Element`/`catch: (e: Error) => …` (LT-208), and the factory context is typed by an annotated second parameter — `, { host, expose }: FactoryContext<Props>` — vocabulary-checked by the compiler (LTC049/050) with the wide ambients as the migration-period fallback (LT-209); authored `.tsrx` is checked emit-then-check through the span table and keeps the ambient vocabulary. Generated modules always go through the span table.
