...
- **CSS**: `.tsx` wraps the stylesheet in the `css` template tag — ``<style>{css`…`}</style>`` — so editors highlight it as CSS; a bare template literal is accepted. `.tsrx`'s `<style>` block is raw CSS. Both emit verbatim, dedented. Scoping is ruled directionally by [ADR 0033](adr/0033-scope-component-styles-by-custom-element-name.md) (🔄 Proposed, backlogged to the [roadmap](ROADMAP.md) — likely 3.1 at TSRX 1.0): tag-name nesting is the model (the convention every stylesheet already leads with), an unprefixed selector IS the global-rule escape hatch, Shadow DOM via declarative shadow root is the sanctioned per-component opt-in (the runtime is already shadow-aware — `first()`/`all()` query `host.shadowRoot ?? host`; what's unbuilt is the compiler's declarative spelling), upstream 0.2's hash-class stamping is declined, and style composition per upstream's pattern — `class={theme.dark}` translated to `my-element .dark` / `:host(.dark)` — is the adopted backlog direction. Until it lands, styles emit verbatim and custom properties are the composition mechanism.
