...
- **Passive event listeners throttled at the input level**: `on()` and `createEventsSensor()` now wrap passive event listeners (scroll, resize, wheel, touch events) with `throttle()` rather than deferring the signal update with `schedule()`. The handler itself runs at most once per animation frame with the latest event, preventing unnecessary churn in the signal graph between frames. This also fixes a bug where two passive event types on the same element inside `createEventsSensor()` would compete for the same schedule slot — only the last one to fire in a given frame would run. Both now run independently once per frame.
- **`createEventsSensor()` cleanup uses `AbortController`**: The internal `Map` tracking registered listeners for cleanup is replaced by a single `AbortController`. Its signal is passed to both `addEventListener` and `throttle()`, so a single `controller.abort()` call handles full lifecycle teardown. No user-facing API change.
