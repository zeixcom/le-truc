...
- **`form-textbox` migrated from manual validity relay to ElementInternals**: the example component now uses `{ formAssociated: true }`, `internals?.setFormValue(v)` for form submission, and `internals?.setValidity({ customError: !!error }, error)` for native Constraint Validation. The manual `host.error` → `setAttribute('aria-invalid')` → `setAttribute('aria-errormessage')` relay on the inner input is replaced by `aria-invalid`/`aria-errormessage` on the host. The `name` attribute moves from the inner `<input>`/`<textarea>` to the `<form-textbox>` host. An `onFormReset` handler clears value and error on form reset. The `error` property remains a public writable reactive prop.
- **`form-combobox` migrated from transitive hidden-input hack to ElementInternals**: the example component now uses `{ formAssociated: true }`, `internals?.setFormValue(v)`, and `internals?.setValidity()` — same pattern as `form-textbox`. The hidden `<input type="hidden">` elements inside the child `<form-listbox>` elements (the transitive hack) have been removed from the HTML. The `on(listbox, 'change', ...)` handler that listened for the hidden input's change event is replaced by `on(listbox, 'click', ...)` that checks for `[role="option"]` directly. The `name` attribute moves from the inner `<input>` to the `<form-combobox>` host.
- **`form-spinbutton` migrated from vestigial input to ElementInternals**: the example component now uses `{ formAssociated: true }`, `internals?.setFormValue(String(v))` for form submission, and `internals?.setValidity({ rangeOverflow, rangeUnderflow })` for native range validation. The inner `<input class="value">` loses its `name` attribute (becomes display-only); the `name` moves to the `<form-spinbutton>` host. An `onFormReset` handler restores the value to `0`.
- **`form-colorgraph` migrated from three separate inputs to single form control via ElementInternals**: the example component now uses `{ formAssociated: true }` and `internals?.setFormValue(formatCss(color))` to submit one serialized `oklch(...)` string. The three `<input name="lightness/chroma/hue">` elements lose their `name` attributes (become display-only); a single `name` attribute moves to the `<form-colorgraph>` host. The per-axis `errors` state and per-input `setCustomValidity`/`aria-invalid` relay are replaced by a single host-level `error` state with `internals?.setValidity({ customError })` and `aria-invalid`/`aria-errormessage` on the host. An `onFormReset` handler restores the initial color. A new spec file (`form-colorgraph.spec.ts`) covers form submission and reset.
