...
- **ElementInternals support via `{ formAssociated: true }` option and managed form-control convention**: `defineComponent` now accepts an optional third `options` parameter — `defineComponent(name, factory, { formAssociated: true })`. A form-associated component exposes a reactive `value` property, and the library manages everything mechanical: form value sync (`value` → `internals.setFormValue(String(value))` via an internal effect in the same deferred-activation pipeline as author effects), `formResetCallback` (restores `value` to its default by re-running the retained initializer — re-parsing the `value` attribute for a `Parser`, native `defaultValue` semantics), `formStateRestoreCallback` (assigns string state to `value`), and `formDisabledCallback` (writes the effective disabled state into a managed `disabled` signal). A typical form component writes **zero ElementInternals code**. See [ADR 0016](adr/0016-element-internals-for-form-association-and-states.md).
- **Native-parity host contract for form-associated components**: the generated class defines `form`, `name`, `labels`, `validity`, `validationMessage`, `willValidate`, `checkValidity()`, `reportValidity()`, and `setCustomValidity()` on the host — delegating to `internals`. External consumers can call these exactly as on a native `<input>`. The managed `setCustomValidity()` resolves a validation anchor (first focusable form-control descendant, falling back to the host) so the browser can focus the control and show the validation bubble. CSS can use native `:disabled`, `:invalid`, `:user-invalid`.
- **Managed `disabled` property**: form-associated hosts get a library-managed reactive `disabled: boolean` property whose setter reflects to the `disabled` content attribute. `formDisabledCallback` writes the *effective* disabled state (including `<fieldset disabled>` inheritance) into the backing signal, so `host.disabled` and `watch('disabled', …)` are correct even for fieldset-inherited disabling. `disabled` is a reserved prop name on form-associated components; `expose({ disabled: … })` throws `InvalidPropertyNameError`.
- **`FormAssociatedElement` interface and overloaded `defineComponent`**: the exported `FormAssociatedElement` interface covers the managed members for author tag-name-map declarations (`'my-input': FormAssociatedElement & MyProps`). When the third argument is the inline literal `{ formAssociated: true }`, the overload requires `P extends { value: string | number }`, types `host` as `FormAssociatedElement & P`, and lets `watch('disabled', …)` typecheck. `value` is deliberately omitted from `FormAssociatedElement` (it is component-exposed, string or number).
- **`internals` on `FactoryContext`**: the `ElementInternals` object (or `null` if `attachInternals()` failed) is exposed as the escape hatch for typed validity flags (`internals.setValidity({ rangeOverflow: true }, msg, anchor)`), custom `:state()` pseudo-classes (`internals.states`), and two-argument `setFormValue(value, state)`. `attachInternals()` is called unconditionally in the constructor (not gated on `formAssociated`) so `:state()` and ARIA reflection are available to all components. A `DEV_MODE` warning fires once per instance on first access when `internals` is `null`.
- **Managed-name collision guard**: on a form-associated component, `expose()` throws `InvalidPropertyNameError` for any managed member name (`form`, `name`, `labels`, `validity`, `validationMessage`, `willValidate`, `checkValidity`, `reportValidity`, `setCustomValidity`, `disabled`). The check runs before the `prop in this` guard, which would otherwise silently skip the colliding initializer. `value` is the deliberate exception. Non-form-associated components are unaffected.
- **`ComponentOptions` type export**: the options object type for `defineComponent`'s third parameter is now exported from the package root.
- **`bindState(internals, token)` binding for custom `:state()` pseudo-classes**: mirrors `bindClass(element, token)` — `value=true` adds the token to `internals.states`, `value=false` removes it; `null` internals is a no-op (graceful degradation). Custom states are component-owned styling hooks: unlike a class, they cannot be clobbered by consumer code rewriting the host's `class` attribute, and they work on any component (internals is attached unconditionally), not only form-associated ones. See [ADR 0016](adr/0016-element-internals-for-form-association-and-states.md) §8.

### Changed

- **`form-listbox` migrated to managed form-control convention**: the example component now uses `{ formAssociated: true }` and the managed convention. The hidden `<input type="hidden">` hack and manual `setAttribute('value', v)` + `internals.setFormValue(v)` + `onFormReset` are gone — form value sync and reset are library-managed. The component dispatches a native-parity `change` event from the host at selection commit (restoring the composition channel `form-combobox` previously got from the hidden input's bubbling `change`). The latent form-reset bug is closed without any component code.
- **`form-textbox` migrated to managed convention; `error` prop retired**: the example component now uses `{ formAssociated: true }`. The public `error` reactive property is removed — external consumers read `host.validationMessage` / `host.validity` (native parity); the component relays the inner input's `validationMessage` via `host.setCustomValidity()`. Inline error text binds to internal state. The manual `aria-invalid` / `aria-errormessage` relay is replaced by native `:invalid` / `:user-invalid`.
- **`form-combobox` migrated to managed convention; `error` prop retired**: same pattern as `form-textbox`. The component now listens to the listbox's host `change` event (native-parity commit) instead of reaching into its DOM with `closest('[role="option"]')`.
- **`form-spinbutton` migrated to managed convention**: form value sync and reset are library-managed. Typed range validation flags (`rangeOverflow` / `rangeUnderflow`) via `internals.setValidity()` are retained — the one legitimate direct-`internals` use among the form examples.
