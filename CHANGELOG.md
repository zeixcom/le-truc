...
- **Generated `docs/` site is no longer committed**: `docs/` is now gitignored (like `custom-elements.json`) and built in CI — `static.yml` runs `bun run build:docs` on a fresh checkout before uploading the Pages artifact, and `ci-cd.yml` builds it before the Playwright tests (which are served from `docs/`; `serve:examples` only rebuilds `docs/assets`, not the HTML pages). This ends generated-artifact churn in PRs and guarantees the deployed site always matches `docs-src/` — the previously committed tree had accumulated 142 stale files from an older API-docs layout (top-level `docs/classes/`, `docs/functions/`, `docs/type-aliases/`, `docs/variables/` — since moved under `docs/api/`) and removed components, which GitHub Pages was still serving. Hand-authored static assets (`favicon.ico`, `assets/img/**`) moved to `docs-src/static/` and are copied into `docs/` by the new `staticAssetsEffect` (`server/effects/static-assets.ts`).

### Fixed

- **Fresh docs build no longer silently skips `sw.js` generation**: `serviceWorkerEffect` hashes `docs/assets/main.css`/`main.js`, but all build effects start in parallel — on a clean `docs/` the assets didn't exist yet on the effect's first run, so it caught the read error, logged it, and the build "succeeded" without a service worker. Masked until now because the committed assets were always on disk. The effect now awaits the CSS/JS effects' first completion (`assetsReady` parameter wired in `server/build.ts`) before reading the assets.
