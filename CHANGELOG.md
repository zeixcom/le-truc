...
- **Bundle-size gate and CI guardrails**: a new `test/regression-bundle.test.ts` (run via `bun run check:size` and as a `Bundle size regression` CI step) enforces REQUIREMENTS.md §4 — it builds a fresh minified bundle in-memory via `Bun.build` with `process.env.DEV_MODE` defined to `false` (matching `build:prod`) and gzips it with `node:zlib`, failing above the 14 kB hard ceiling and warning above the 10 kB target. It deliberately does *not* measure the published `index.js`, which is unminified as of this release (see Changed below) — the gate measures what a consumer's bundler ships. `ci-cd.yml` and `codeql.yml` now trigger on `[main, next]` instead of `[main, develop]` (`develop` no longer exists; `next` — the active integration branch — previously received zero CI runs), and `ci-cd.yml` gains a `Run server tests` step (`bun run test:server`, 31 test files that previously ran nowhere in CI).
- **Doc link checker (`check:links`)**: `scripts/check-doc-links.ts` validates internal links and heading anchors across `adr/*.md`, `ARCHITECTURE.md`, `REQUIREMENTS.md`, and `AGENTS.md` — resolving relative paths from each file's directory, slugifying destination headings GitHub-style, and asserting every `#fragment` matches one. Run via `bun run check:links`; wired as a `Check doc links` CI step in `ci-cd.yml` after Lint.

### Changed

- **`@zeix/cause-effect` upgraded to `^1.4.0`** from `^1.3.4`: adds `EffectConvergenceError` (thrown when state mutations in effects do not converge within 1000 iterations) and `InvalidStoreMutationError` (thrown when directly mutating a store proxy — use `.add()`, `.set()`, `.update()`, `.remove()` methods instead), both re-exported from Le Truc's `index.ts` alongside the rest of the Cause & Effect re-exports.
- **Package distribution metadata overhauled** (`package.json`): The published entry point is now the *unminified* ESM bundle (`index.js`) instead of a minified artifact — consumers' bundlers minify anyway, and readable source improves debugging and bug reports; `build:prod` accordingly drops `--minify` and `--sourcemap=external`, and the tracked `index.js.map` is removed. An explicit `exports` map (`.`: `types` → `bun` → `default`, plus `./package.json` and `./custom-elements.json`) replaces the bare `main`/`module` pair: the `"bun"` condition resolves TypeScript source directly for Bun consumers, while `"default"` serves the bundled `index.js` to all other toolchains. The `"module": "index.ts"` field is removed — it pointed at raw TypeScript, which broke webpack and older Rollup configs. TypeScript is now an *optional* peer dependency (`peerDependenciesMeta`), so JS-only consumers no longer get an unresolvable-peer warning. A `files` allowlist (with `!src/tests` / `!types/src/tests` negations to keep the test suites out of the tarball) replaces `.npmignore`; the unused `index.dev.js` artifact and its `build:dev` script are removed (examples build from `index.ts` with their own `DEV_MODE` define). Because `custom-elements.json` is gitignored but allowlisted, `npm-publish.yml` now runs `bun run build:cem` before publishing. **Migration:** The `exports` map blocks deep imports into package internals — code relying on these must switch to named imports from the package root. If your toolchain read the `module` field to consume TypeScript source, switch to the `"bun"` exports condition or import the bundled `index.js`. If you consumed `index.dev.js`, build from `index.ts` with `process.env.DEV_MODE` defined to `"true"` instead.
- **Generated `docs/` site and TypeDoc output are no longer committed**: `docs/`, `docs-src/api/`, and `docs-src/pages/api.md` are now gitignored (like `custom-elements.json`) and built in CI — `static.yml` runs `bun run build:docs` on a fresh checkout before uploading the Pages artifact, and `ci-cd.yml` builds it before the Playwright tests (which are served from `docs/`; `serve:examples` only rebuilds `docs/assets`, not the HTML pages). This ends generated-artifact churn in PRs (TypeDoc rewrites commit-hash source links in every `docs-src/api` file on each build) and guarantees the deployed site always matches the source — the previously committed `docs/` tree had accumulated 142 stale files from an older API-docs layout (top-level `docs/classes/`, `docs/functions/`, `docs/type-aliases/`, `docs/variables/` — since moved under `docs/api/`) and removed components, which GitHub Pages was still serving. Hand-authored static assets (`favicon.ico`, `assets/img/**`) moved to `docs-src/static/` and are copied into `docs/` by the new `staticAssetsEffect` (`server/effects/static-assets.ts`).

### Fixed

- **`requestContext` no longer permanently locks in the fallback when the provider connects late**: the consumer dispatched a single `ContextRequestEvent` synchronously during its factory run and then permanently served whatever it got — if no ancestor provider had answered at that instant, the returned signal served the `fallback` forever, with no recovery. A provider realistically misses that instant when its `customElements.define()` runs after the consumer's (bundle ordering, code-split chunk, deferred script) or when it is upgraded but its own `provideContexts` listener hasn't activated yet (descriptors activate after dependency resolution — see [ADR 0007](adr/0007-effect-descriptors-with-deferred-activation.md) — which involves at minimum a `queueMicrotask` and possibly a 200 ms `customElements.whenDefined()` wait). The returned `Signal<T>` is now backed by a `Slot` (the same primitive `pass()` uses for overridable backing signals), initially delegating to a `State` holding the `fallback`; when a provider answers, its getter `() => host[context]` is wrapped in a `Memo` (so the provider's underlying signal is tracked) and swapped in via `slot.replace()`. The `Slot`'s own computation reads the delegated signal inside a tracking context, so a single read tracks both the swap and the provider's live value — the same primitive, unified with `pass()` for all maybe-overridable signals. The request is re-dispatched once on a microtask and once after the dependency-resolution window (`CONTEXT_RETRY_DELAY = 210` ms, deliberately exceeding `DEPENDENCY_TIMEOUT = 200` ms in `src/helpers/dom.ts`). **Type change**: `requestContext`'s return type widens from `Memo<T>` to `Signal<T>` — a `Slot` satisfies `Signal<T>`, and consumers only read the value, so this is source-compatible for all existing usage inside `expose()`. **Behavioral change**: a consumer that previously rendered the fallback permanently (a silent failure) now binds to the provider's value once the provider upgrades; if no provider answers within ~210 ms, the fallback remains permanent for that connection (and a `DEV_MODE` warning names the context and host). See [ADR 0015](adr/0015-late-provider-retry-in-requestcontext.md).
- **Collection changes no longer rebuild every surviving element's scope in `each()`, `pass()` with Memo targets, and the non-bubbling per-element `on()` fallback**: all three helpers previously wrapped their per-element `createScope` loop in a `createEffect`, and an effect disposes every owned child scope before each re-run — so one element entering or leaving a `Memo<Element[]>` collection tore down and recreated the scopes of **all** N elements: event listeners detached and re-attached, `pass()` restored every child's original slot signal and swapped in a freshly created computed (re-running every child's downstream effects), and DEV_MODE `pass()` deprecation warnings re-fired for every element on every change — O(n) churn per mutation in a 1000-row list. Per-element scopes are now keyed by element identity via a shared internal `keyedScopes()` helper (`src/helpers/reactive.ts`): only elements that **enter** the collection get a new scope (`createScope` with `{ root: true }`, so the driving effect's re-run cannot dispose it), only elements that **leave** get exactly their own scope disposed, and surviving elements' scopes — listeners, slot swaps, nested effects — are untouched. Child components bound via `pass()` no longer see their slot signal replaced (and their downstream effects re-run) when an unrelated sibling enters or leaves. An enclosing wrapper scope still disposes every live per-element scope on component disconnect, and a pure reorder of the same element set creates and disposes nothing. Throw semantics per [ADR 0011](adr/0011-throw-on-pass-binding-failure.md) are preserved — an unbindable `pass()` prop now throws when its element *enters*, and a throw never registers the failed element's scope. See [ADR 0014](adr/0014-keyed-per-element-scopes-for-memo-collections.md).
- **One-shot docs build (`build:docs`) is now correct from a fresh checkout**: with the generated inputs gitignored, three latent races in the reactive build surfaced — all rooted in every effect starting in parallel and taking its first-run snapshot before the generators had produced anything. (1) `serviceWorkerEffect` read `docs/assets/main.{css,js}` before the CSS/JS effects wrote them, caught the error, and the build "succeeded" without `sw.js`. (2) The `docs-src/api` file signal scanned before TypeDoc ran (and `watchFiles` attaches no fs watcher when the directory doesn't exist at startup), so `docs/api/` came out empty. (3) `pagesEffect` consumes an async computed, whose re-run after `api.md` was written settled only after `buildOnce` had resolved every `ready` promise and unsubscribed the effects — `docs/api.html` was never generated. `server/build.ts` now runs in two phases: generators first (`apiEffect`, `cssEffect`, `jsEffect`, `staticAssetsEffect`), then consumers, which are only instantiated after phase 1 completes; `apiEffect` additionally pushes TypeDoc's output through the file signals via the new `watchFiles` `rescan()` escape hatch (`server/file-watcher.ts`). Previously masked because the committed artifacts were always on disk. Watch-mode incremental rebuilds are unchanged.
...
- **ADR and REQUIREMENTS.md cross-reference rot repaired** (no runtime impact — documentation only)
