...
- **`createCell`, `deriveCell`, `DeriveCellOptions` re-exported**: the CE 1.5.1 terminal names, added to `index.ts` alongside the `deriveSignal`/`DeriveSignalOptions` names they replace. `createCell` is an alias of `createState`; `deriveCell` dispatches the same way `deriveSignal`/`createComputed` did (sync callback → `Memo`, async callback → `Task`, both returned as `Signal<T>`).

### Changed

- **`@zeix/cause-effect` upgraded to `^1.5.1`** from `^1.5.0`: additive upstream release that renames `deriveSignal` to `deriveCell` (deprecating `deriveSignal` after a single release) and deprecates the `Memo`/`Task` types, `isMemo`/`isTask`/`isComputed`, ahead of CE 2.0 (see Deprecated below).
...
- **`deriveSignal`/`DeriveSignalOptions`, and every name `@zeix/cause-effect` 1.5.1 continues to deprecate ahead of its 2.0, mirrored on Le Truc's re-export surface**: `deriveSignal` and `DeriveSignalOptions` — themselves the `2.5.0` bridge names — are deprecated as of CE 1.5.1 in favor of `deriveCell`/`DeriveCellOptions`. `createComputed`, `isComputed`, `isMemo`, `isTask`, and the `Memo`/`Task` types remain deprecated as before, still inherited onto `@zeix/le-truc`'s re-exports from CE's own declarations. `Memo`/`Task`/`isComputed`/`isMemo`/`isTask` have **no mechanical replacement** — origin is no longer part of the consumption contract in CE 2.0; annotate with `Signal<T>` instead of `Memo<T>`/`Task<T>`, and use `isSignal()`/`isMutableSignal()` or a plain property check in place of the origin guards. **Migration**: `deriveSignal` → `deriveCell` (pure rename, same dispatch), `createComputed` → `deriveCell` (manual — `options.value` becomes `options.initial`). All deprecated re-exports keep working through Le Truc 2.x and are removed in Le Truc 3.0, which follows CE 2.0.
...
- **Cause & Effect 2.0 bridge names, re-exported**: `deriveSignal`, `deriveList`, `deriveStore`, the utilities `isPending` and `abort`, the predicates `isMutableList`, `isDerivedList`, `isMutableStore`, and the types `MutableList`, `DerivedList`, `MutableStore`, `DerivedStore`, `MutableSignal`, `DeriveSignalOptions`, `DeriveListOptions`, `DeriveStoreOptions`, `StoreCallback`, `ListSource`, `ListCallback`, `ListChanges`, `PerItemCallback`. Code written against these names survives CE 2.0: the `Mutable*` family, `MutableSignal`, and the `List*`/`PerItem` callback vocabulary are the terminal v2 names (only `DerivedList`/`DerivedStore` rename once more at 2.0, to the readonly `List`/`Store` bases). `isPending(signal)` replaces the `isTask(s) && s.isPending()` spelling and tracks reactively when read inside a computation; `abort(signal)` returns the `AbortSignal` of an in-flight async computation.
- **"Prepare for Cause & Effect 2.0" documentation**: new section in the Components guide with the full rename table and the terminal-vs-renames-again distinction, and a new "Fetch Data into a List" section in the Data Flow guide composing `deriveList(asyncFn, { initial, keyConfig })` with `reconcile()` and `isPending()` — including the thunk adapter `deriveList(() => task.get(), { keyConfig })` for an existing `Task` (passing the signal directly does not type-check: a signal first argument selects `deriveList`'s per-item-callback overloads). Both link CE's `MIGRATION-2.0.md` and document the codemod invocation for Le Truc consumers: `bun tools/codemod-v2.ts 'src/**/*.ts' --module @zeix/le-truc`.

### Changed

- **`@zeix/cause-effect` upgraded to `^1.5.0`** from `^1.4.1`: additive upstream release that back-ports the v2 bridge names and deprecates the names they replace (see Deprecated below).
- **`reconcile()` type annotations renamed from `List`/`Collection` to `MutableList`/`DerivedList`**: the deprecated names are exact aliases of the same types, so the change is signature-identical and every existing call site compiles unchanged — the annotations simply stop using doomed vocabulary. The deprecated aliases remain valid `reconcile()` sources.
- **`each()`, `pass()`, and `keyedScopes` element-collection annotations widened from `Memo<E[]>` to `Signal<E[]>`**: in CE 1.5, `Signal` is still the umbrella union, so this is a pure type widening. `pass()`'s runtime guard moved from `isMemo<Q[]>()` to `isSignal<Q[]>()` in lockstep, so the newly type-valid `Signal<Element[]>` targets route into the keyed per-element branch instead of falling through to the single-element branch — behavior-identical for every previously valid input, and already correct under v2 semantics, where that union is single-value by construction.
- **Internal `createComputed` usage replaced with `deriveSignal`**: `toSignal()` in the reactive helpers and `#setAccessor` in `component.ts` now derive through the non-deprecated name with identical sync/async dispatch (`deriveSignal` auto-detects async thunks exactly as `createComputed` did). No deprecated Cause & Effect API remains in Le Truc's own source.
- **Examples migrated to the bridge names**: `module-list` and `module-todo` annotate with `MutableList`/`MutableStore`, and `module-calctable` replaces the deprecated `list.deriveCollection(fn)` method chain with `deriveList(list, fn)` (the CE codemod deliberately skips method chains, so that call was rewritten by hand). No deprecated names remain anywhere under `examples/`.
- **`cause-effect` skill (`SKILL.md`, `references/primitives.md`, `references/pitfalls.md`)**: grounded on CE 1.5.0. The primitive picker, `deriveList` flavors, and predicate guidance now lead with the bridge names, mark the deprecated aliases, and distinguish terminal v2 vocabulary from names that rename once more at 2.0.
- **`le-truc` and `le-truc-dev` skill references**: re-export examples, the `expose()` initializer table, and internal-behavior notes use `deriveSignal` and the bridge names; stale `createComputed`/`createCollection`/`deriveCollection` spellings removed.
- **`docs-server-dev` references and the docs build pipeline**: the build effects share one extracted `createBuildEffect` wrapper instead of per-effect scaffolding, with `references/effect-pattern.md`, `references/source-map.md`, and `server/SERVER.md` updated to match; Markdoc children arrays are typed as `NodeType[]` in the pages pipeline.

### Deprecated

- **Every name `@zeix/cause-effect` 1.5 deprecates ahead of its 2.0, mirrored on Le Truc's re-export surface**: importing `createComputed`, `createMutableSignal`, `List`, `isList`, `Collection`, `isCollection`, `createCollection`, `Store`, `isStore`, or the origin types and guards `State`, `Memo`, `Task`, `Sensor`, `isState`, `isMemo`, `isTask`, `isSensor`, `isComputed` from `@zeix/le-truc` now shows the same deprecation strikethrough as importing them from CE directly. The markers are inherited from CE's declarations through the re-export aliases — re-declaring JSDoc per-specifier is not expressible in an export block, and the editor experience resolves identically. **Migration**: `createComputed` → `deriveSignal` (manual — not covered by the codemod; `options.value` becomes `options.initial`), `createMutableSignal` → `createSignal`, `List`/`isList` → `MutableList`/`isMutableList`, `Collection`/`isCollection`/`createCollection` → `DerivedList`/`isDerivedList`/`deriveList`, `Store`/`isStore` → `MutableStore`/`isMutableStore`, origin types and guards → `isSignalOfType()`. Follow CE's [`MIGRATION-2.0.md`](https://github.com/zeixcom/cause-effect/blob/main/MIGRATION-2.0.md); run the codemod with `--module @zeix/le-truc` since Le Truc consumers import through the Le Truc entry point. All deprecated re-exports keep working through Le Truc 2.x and are removed in Le Truc 3.0, which follows CE 2.0.

## 2.4.0

### Added

- **`DEV_MODE` debug extension**: `defineComponent()` now auto-appends an internal `debug()` extension to every component whenever `process.env.DEV_MODE === 'true'`. It adds a reactive `debug: boolean` property (default `false`), synced to `:state(debug)`, and toggleable at runtime by `metaKey`+click on the nearest custom-element ancestor (a single lazily-installed, capture-phase document listener). While `debug` is `true` on a component, every `on()`/`pass()`/`watch()` firing in its scope logs one `console.debug` entry naming the firing kind, the host, and the attributed target element, and pulses a CSS box-shadow animation on both the host and the target — target attribution comes from `bind*` helper (`bindText`, `bindProperty`, `bindClass`, `bindVisible`, `bindAttribute`, `bindStyle`) registers. Everything is additionally gated at its own call sites, so a bundler that fails to fold `DEV_MODE` still can't ship this live in production — the primary guard is `component.ts` never appending the extension outside `DEV_MODE` at all. See [ADR 0022](adr/0022-debug-extension-for-visual-and-console-instrumentation.md).
...
- **`ComponentExtension` interface and extension-based dependency injection for `defineComponent()`**: the third parameter is now `readonly ComponentExtension[]`, so `component.ts` never statically imports feature-specific code. Every bundled extension lives in `src/extensions/` and is tree-shaken away unless a consumer actually imports and uses it. An extension is a plain data/function bundle (`name`, `staticProps`, `observedAttributes`, `reservedMembers`, `installOnPrototype`, `onConnect`, `onAttributeChanged`); `mergeExtensions()` folds the array once at class-definition time — `staticProps` collisions throw the new `ExtensionCollisionError` in DEV_MODE and first-wins in production, while `observedAttributes`/`reservedMembers` are unions. `component.ts`'s `#initSignals` now retains every exposed prop's initializer so extensions can read back what they need (e.g. `formAssociated()`'s reset re-runs the retained `value` initializer; `observedAttributes()` re-runs a retained `Parser`). See [ADR 0019](adr/0019-extension-based-dependency-injection-for-definecomponent.md).
- **`formAssociated()` extension and managed form-control convention**: `defineComponent(name, factory, [formAssociated()])` exposes a reactive `value` property and the library manages form value sync, reset, state restore, and `<fieldset disabled>`-aware `disabled` state. The generated host also gets a native-parity contract (`form`, `name`, `labels`, `validity`, `checkValidity()`, `setCustomValidity()`, …) delegating to `internals`, so a typical form component writes **zero ElementInternals code**. `expose()` guards against redeclaring any reserved member name. Migrated from the v2.3.0-beta.1 `{ formAssociated: true }` option before release. See [ADR 0016](adr/0016-element-internals-for-form-association-and-states.md) and [ADR 0019](adr/0019-extension-based-dependency-injection-for-definecomponent.md).
- **`formAssociatedCheckbox()` extension**: a special form of `formAssociated()` for checkbox-shaped controls whose primary state is `checked: boolean` rather than `value: string | number`. It shares the shape-agnostic host contract, `disabled` management, and validation helpers with `formAssociated()`, but the value-sync/reset/state-restore triad differs: `internals.setFormValue(checked ? submitValue : null)` (submitting nothing when unchecked, matching native `<input type="checkbox">`), a `checked`-target reset, and state-restore as `typeof state === 'string'`. `submitValue` is read once from the host's own `value` attribute at connect (default `'on'`), not a reactive prop — native checkbox `.value` is a static identifier, not the commit signal. **Do not combine with `formAssociated()`** on the same component: both declare the same `staticProps.formAssociated` key, so DEV_MODE throws `ExtensionCollisionError`. Radio groups and listboxes don't need this (their selection already aggregates into one string `value` on the container). See [ADR 0019](adr/0019-extension-based-dependency-injection-for-definecomponent.md).
- **`observedAttributes()` extension**: the opt-in escape hatch ADR 0003 reserved for attribute-driven reactivity — brought back now that v2.0 dropped `observedAttributes`/`attributeChangedCallback` in favor of properties as the reactive interface (motivated chiefly by frameworks like React that set DOM attributes on custom elements rather than properties). Properties remain the primary reactive interface; this extension only re-parses an already-`expose()`d Parser-backed prop when its attribute mutates after connect, re-running the same retained `Parser` against the attribute's new string value. Props whose initializer wasn't a branded `Parser` are left untouched. See [ADR 0019](adr/0019-extension-based-dependency-injection-for-definecomponent.md).
- **`asBoolean()` fallback parameter**: `asBoolean(fallback = false)` now takes an optional fallback value (default `false`, the previous behavior), matching `asInteger`/`asNumber`/`asEnum`/`asString` which already had one. Purely additive — lets a component read the sole source of truth for the boolean value (e.g. `formAssociatedCheckbox()` reading `checked` of native checkbox).
...
- **`FormAssociatedElement` interface and `FormAssociatedExtension` / `FormAssociatedCheckboxExtension` type exports** for typing form-associated tag-name-map declarations and the branded extensions returned by `formAssociated()` / `formAssociatedCheckbox()`.
...
- **`reconcile(container, template, source, bindItem)`**: new top-level primitive that syncs a keyed reactive data source (`List<T>` or `Collection<T>` from `@zeix/cause-effect`) to a container's children — Le Truc's first data-driven DOM creation, and the ownership complement of `each()` (which enhances DOM the component does *not* own). Entering keys clone the `<template>`'s single root element and mount `bindItem(element, item, key)` in a root-keyed scope with **collector parity to `each()`'s callback** (the ADR 0014 ownership discipline): `watch()`, `on()`, `pass()`, and `each()` may be called inside it directly, and the collected descriptors activate against that per-item scope rather than the driving structural effect — so an item-level `watch(item, …)` never makes structural work depend on item signals. Leaving keys dispose their scope, then their element is removed; survivors are moved with `insertBefore()` — always reused, never recreated. The sync is strictly one-way, data → DOM. The first run **adopts** server-rendered children carrying `data-key` (`bindItem` runs for adopted elements too and is responsible for its own idempotency); keyed children not in the source and all unkeyed children are removed (self-cleaning container). Children carrying `data-unreconciled` are exempt — never removed, never repositioned, no `bindItem` — a permanent public SSR/interaction contract (drag-and-drop markers, mid-stream server items); an element `reconcile()` itself placed that later gains the attribute still claims its key, so a mid-drag re-run cannot clone a duplicate. Keyed elements are positioned relative to the *keyed subset*, not absolute child index, so unmanaged elements interspersed in the container do not drift keyed positions. The driving effect tracks structural changes only (the source's keys); per-item value changes flow through the `byKey` signal passed to `bindItem`. See [ADR 0017](adr/0017-keyed-template-clone-reconciliation-for-lists.md).
