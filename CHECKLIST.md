...
- [ ] **DOC — the arm is a compiler decision, never a microtask outcome.** Under Server Simulation (ADR 0027) the driver serializes synchronously — no `await` between instantiation and serialization — so nothing but the compiler's decision determines which arm ships. A resolution phase *before* the simulation may await deliberately; that is where a build-time data dependency belongs.
