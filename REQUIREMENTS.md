...
Every component's reactive initial values are resolved server-side by the cheapest mechanism that can actually answer them, decided statically at compile time ([ADR 0029](adr/0029-tiered-server-evaluation.md)): the **Folded** tier (DOM-less value harness), the **Simulated** tier (Server Simulation, [M20](#m20-server-simulation-realm)), and the **Static** tier (skeleton only; the client corrects). Unresolvability is a per-expression property — an expression no phase can answer (layout geometry, stubbed internals surfaces, wall clock, RNG, runtime-default locale) is omitted in every tier, and the client supplies it at connect. **A serialized state payload is avoided by default**: the served HTML is corrected at connect, never hydrated, and no framework-managed whole-app state blob ships. Where serialization is genuinely unavoidable — configuration for a client-side derivation being the standing case — the preferred form is a **component-local payload on one of the component's own attributes**, parsed by `asJSON`. The rule is a discipline about scope and authorship, not a ban on serialized data: per-component authored config is in scope; an automatically synthesized hydration blob is what this forbids, and a front end that generates one has failed the project's size bet ([ADR 0032](adr/0032-adopt-tsx-as-the-authored-component-surface.md), amended 2026-09-19).
...
The compiler emits, alongside the client module and the CSS, a **partial with holes** for consumption by a non-JS backend ([ADR 0034](adr/0034-distribution-tsx-only-compiler-package-and-template-emission.md) s3): every prop-independent expression folded, every server arg emitted as a variable in a target template language. **Twig is the v3.0 target**; HTL (AEM) follows. Targets sit behind a **target-emitter interface decided before the first emitter is written**, with Twig as its first implementation — a second target must be addable without reshaping the first (LT-257). Each target owns an explicit escaping contract — the emitter places the target language's escaping at every hole, and a hole in a position the target cannot escape safely is a compile-time diagnostic ([M22](#m22-tiered-error-surfacing), Prevented), never a silently unsafe emit.
...
- Baseline policy (owner, 2026-09-24): each **major release of the runtime** (`@zeix/le-truc`) pins the baseline to the year three years before its release (3.0 → 2023). **Minor and patch releases never move it**, however much time has passed; only the next runtime major may. The compiler (`@zeix/le-truc-compiler`) versions independently, and its default output stays within the baseline of the runtime major its peer range names, so a compiler major never moves the baseline ([ADR 0034](adr/0034-distribution-tsx-only-compiler-package-and-template-emission.md) s8). A build-time check fails when shipped code uses a feature newer than the pinned year (LT-305).
