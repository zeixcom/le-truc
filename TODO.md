...
- [x] LT-053: Rename the `pass` special attribute to the namespaced `truc:pass` (CHECKLIST §1) — done, pending review ⏳
  **Skill:** le-truc-dev
  **Changed:** `ast-utils.ts` (`attrName` flattens `JSXNamespacedName`); `classify-attributes.ts` (accepts `truc:pass`, warns on bare `pass`); `diagnostics.ts` (TSRX021, warning); codemod of 6 sites across 4 example files and 3 test files; 3 new tests in `pass.test.ts`.
  **How:** Verified by probe that the pinned parser accepts `truc:pass` and yields a proper `JSXNamespacedName` before writing anything. `jsxName` is deliberately left alone — it also resolves element tags, where a namespace means something else — so the flattening lives in `attrName`. Bare `pass` warns rather than errors: it is load-bearing across the migrated corpus (LT-016/LT-017), and only TSRX001 nulls a component, so a warning does not gate the build.
  **Check:** The removal cycle for bare `pass` is not scheduled — TSRX021 says "removed next" but nothing tracks it. Worth a follow-up task, or drop the promise from the message.
