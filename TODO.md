...
**ADR 0027 wave — Server Simulation. CURRENT, and the spikes are closed.** LT-149/LT-150 shut
the two engine gaps, LT-151 proved the driver portable (Bun/Node/Deno, byte-identical), and
LT-152 confirmed the substrate. Both are reviewed as of 2026-09-03. **The remaining wave is
implementation, not investigation:** LT-154 builds the stage-1 driver, LT-163 gives it a
diagnostic channel, LT-164 gives it a golden strategy. The ADR acceptance milestone is no
longer blocked on evidence — it is blocked only on LT-153's plan, which this section records.
...
3. **LT-142 removed a bug's diagnostic without removing the bug, and it is shipping now.**
   ...
   markup *silently*. This is ADR 0027's "the diagnostics loss is structural" concern, already
...
2. **The warning baseline is 2, not 8, and it is the wrong instrument after LT-154.** LT-146
...
5. **LT-142 has a service life and it is not over.** The `Intl` fold rule is part of the
   ...
   basis** (see its archive line); it retires with LT-165, not before.
...
3. **LT-146** — `form-tokenbox.description` stops shipping through two channels. Component-only,
   no compiler change, and it is one of the two warnings still standing. TSRX039 survives LT-153
   intact, so this task is unaffected by the wave.
...
- [ ] LT-163: Give the simulation driver a build-report channel, and re-baseline the wave-4 regression signal on it.
  **Skill:** docs-server-dev
...
- [ ] LT-164: Golden strategy for simulated renders, plus the two-order and double-connect invariants.
  **Skill:** docs-server-dev
...
- [ ] LT-166: Memoize simulated renders on `(component, args)`. **TRIGGERED, not scheduled — fires when a migration pushes the simulated build stage past +1.0 s.**
  **Skill:** docs-server-dev
...
- [ ] LT-144: Pin what survives of the silent-empty-render class under simulation.
  **Skill:** docs-server-dev
