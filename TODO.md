...
- [ ] LT-022: Restructure `analyze.ts`'s monolithic `analyzeClient` into explicit passes over a shared context (architecture deepening, deferred from LT-021; = move M5 of `server/tsrx/LE_TRUC_COMPILER.md` §7)
  **Skill:** le-truc-dev
  **Context:** `analyzeClient` (~1400 of `analyze.ts`'s 1951 lines) is one function with ~25 nested closures sharing implicit state — none of its passes (selector addressing, `@for`→`each()` planning, reactive-list→`reconcile()` planning, harvest-plan selection, effect emission) are independently readable or testable. Thread the shared state through an explicit context object (mirroring `compiler.ts`'s own `ExtractContext` pattern) and split each pass into its own module — target layout `analysis/{selectors, naming, harvest, loops, effects, plan}` per LE_TRUC_COMPILER.md §7.2. Sequence after LT-042 (`walk.ts`) and LT-043 (`evaluability.ts`) so the split lands on the consolidated helpers.
...
- [ ] LT-039: Extract the shared IR vocabulary into `ir.ts` (compiler regrouping M1, `server/tsrx/LE_TRUC_COMPILER.md` §7)
  **Skill:** le-truc-dev
  **Context:** `compiler.ts` owns both front-end extraction and the entire IR type vocabulary, so every sibling (classify-attributes, config, plain-imports, analyze, emit-*) imports types from the module that does the work — a type-level hub-and-spoke. Move `TemplateNode`, `AttributeIR`, `ComposeAttrIR`, `PassEntryIR`, `ForIR`, `SignalIR`, `ConfigIR`, `ComponentIR`, `ExtractContext`, `SetupStmt`, `SourceRange` into a new leaf `ir.ts` and re-point the siblings. Pure move; first step of the coupling-reduction plan in LE_TRUC_COMPILER.md §7.
  **Check:** No behavior change; all `server/tests/tsrx` goldens byte-identical; tsc/biome clean.

- [ ] LT-040: Extract `core.ts` — single `@tsrx/core` value-import adapter; kill the compiler⇄lower-template value cycle (compiler regrouping M2)
  **Skill:** le-truc-dev
  **Context:** `compiler.ts` ⇄ `lower-template.ts` is a real runtime value cycle (`lowerChildren` vs the `isForOfNode`/`isVoidTag` re-exports), an artifact of the "one `@tsrx/core` value importer" rule (ADR 0024 sub-design 2). Move all five value imports (`parseModule`, `isStyleElement`, `getStyleElementStylesheet`, `isTemplateForOfNode`, `isVoidElement`) into a leaf `core.ts`; `lower-template.ts`/`emit-server.ts` import predicates from there; delete the re-exports. Pin upgrades then touch `core.ts` + `core-shim.d.ts` only — the same isolation goal, achieved without the hub.
  **Check:** No import cycle remains (verify via bundler graph or tsc); goldens byte-identical; `server/SERVER.md`'s module map (§ TSRX Compiler) updated.

- [ ] LT-041: De-duplicate shared predicates/helpers; bring `CONTEXT_HELPERS` under the vocabulary parity test (compiler regrouping M7)
  **Skill:** le-truc-dev
  **Context:** One rule in several homes across the package: node predicates ×3 (`isNode` / emit-server's `isTsrxNode` / analyze's `nodeType`), host-prop mirror matching ×2 (`hostPropMirrorExpr` / `hostPropMirrorOf`), near-duplicate `classMapKeys`/`styleMapKeys`, `sanitizeVarName` vs compiler.ts's inline copy, emit-server's `reindent` duplicating the algorithm inside `appendWithSpans`, and emit-client's `CONTEXT_HELPERS` as the only context-vs-module vocabulary list NOT covered by `globals.test.ts` parity. Consolidate into ast-utils/spans and add `FACTORY_CONTEXT_MEMBERS` to the parity surface.
  **Check:** Goldens byte-identical; `globals.test.ts` extended to the factory-context list and passing.

- [ ] LT-042: Consolidate the ~12 bespoke `TemplateNode` walks into one `walk.ts` visitor (compiler regrouping M3)
  **Skill:** le-truc-dev
  **Context:** `collectComposeElements`, plain-imports' four generators, `collectRefs`, `countForSelector`, `countComposeBySource`, `parentOf`, `enclosingIfOf`, `findHoleParent`, `findMirror`/`findAttrSite`, `recordSites`, `validateComposedChildren`/`validateListBody` walks, `collectItemEvents`, `gatedLazyChild`, `hasDeepConstruct`/`hasClientConstructs` — each hand-rolls the same recursion, so every new `TemplateNode` variant must be added to all of them. Build one `walkTemplate(node, visitor)` with the traversal rules (if/switch/try/compose/fors) encoded once, plus derived collectors; migrate consumers module by module (plain-imports first, then compiler, then analyze).
  **Check:** Goldens + diagnostics tests unchanged after each migration step; a new unit test exercises the visitor over every node kind.

- [ ] LT-043: Single home for the server-known evaluability rule — `evaluability.ts` (compiler regrouping M4)
  **Skill:** le-truc-dev
  **Context:** The predicate deciding what the server renders (`freeIdentifiers(node)` − `JS_GLOBALS` ⊆ scope, plus its `dependenciesOf` helper defined identically in analyze/emit-server/plain-imports) is restated in emit-server (reactive attrs, class/style maps, `html`, lazy children), mirrored by plain-imports' `walkServerRenderedThunks` — whose own comment says it "mirrors emit-server.ts's own gate exactly" — and preconditioned in analyze's `substituteArgExpr`. Divergence means a wrong component, not a wrong message. Extract one `dependenciesOf` + one `isServerEvaluable(node, scope)` and import it everywhere the gate applies.
  **Check:** Server goldens byte-identical (proves the server render set unchanged); the three modules share one implementation.

- [ ] LT-044: Merge import handling into one module; remove `node:path` (compiler regrouping M6; browser-purity gate for ADR 0025)
  **Skill:** le-truc-dev
  **Context:** `config.ts` holds two unrelated concerns (its own header admits it: config extraction + compose-import resolution), and `config.ts` + `plain-imports.ts` carry the package's only Node APIs (`posix` from `node:path`) — the single obstacle to a browser build (ADR 0025 sub-design 6). Create `imports.ts` owning compose-import resolution + plain-import parse/place; `config.ts` keeps only `readConfig`; replace `posix` with a small pure string helper.
  **Check:** `grep -rn "node:" server/tsrx --include="*.ts"` clean excluding `smoke.ts`; plain-imports/config tests unchanged; goldens byte-identical.
