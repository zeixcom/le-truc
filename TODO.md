...
- [ ] LT-035: Context protocol support — `requestContext()`/`provideContexts()` in `.tsrx` (ADR 0024 sub-design 15, depends on LT-034)
  **Skill:** le-truc-dev
  **Context:** Traces to [REQUIREMENTS.md M10](../REQUIREMENTS.md#m10-context-protocol) — the context protocol is a Must-Have for the library, so `.tsrx` needs to express it. Currently zero support: `requestContext` isn't a recognized signal constructor or ambient (`CONTEXT_NAMES` in `ast-utils.ts` is `host`/`internals` only), so a bare `const motion = requestContext(MEDIA_MOTION, 'unknown')` setup statement is rejected outright. Found migrating `card-mediaqueries.tsrx` (NOTES.md, now resolved), which consumes four ancestor-provided contexts. Depends on LT-034 landing first: `Context` object definitions (`MEDIA_MOTION` etc.) live in plain `.ts` modules that need import passthrough to be usable in `.tsrx` sources at all. Consumer side: add `requestContext` as a new recognized call form alongside `SIGNAL_CONSTRUCTORS` — its result is signal-like downstream (`.get()`, usable in reactive attrs/lazy text same as `createCell`/`deriveCell`), but server rendering uses the call's `fallback` argument (must be server-known — a literal or simple expression) as the initial value instead of actually resolving context server-side (there's no ancestor DOM to walk at render time) — client corrects away from the fallback once the real context resolves, the same accepted flash-risk tradeoff already documented in sub-design 3 for other non-server-known thunk attributes. Provider side: add `provideContexts` to the recognized-ambient-call allowlist next to `host`/`internals` — it's a pure connect-time side effect (`client-stmt`), renders nothing server-side, same treatment as the existing `internals?.states.add(...)` pattern. Re-attempt `card-mediaqueries.tsrx` once this lands as the acceptance case.
  **Check:** New test fixtures covering both directions: a component consuming a context with server-rendered fallback + client correction, and a component providing context to descendants. `card-mediaqueries.tsrx` re-migrates and passes `check:tsrx` end to end. Existing corpus unaffected.
