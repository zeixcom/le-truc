...
- [x] LT-046: (Optional) Extend `walk.ts` coverage to the remaining bespoke `TemplateNode` walks — or document why not — done ✓
  **Skill:** le-truc-dev
  **Summary:** Reviewed all six holdouts (`countForSelector`/`countComposeBySource`, `parentOf`, `findHoleParent`, `findMirror`/`findAttrSite`, `hasDeepConstruct`, `recordSites`) against `walkTemplate`'s pre-order-visit-everything shape. None generalize without losing behavior: `countForSelector`'s max-vs-sum branch-exclusivity arithmetic IS the recursion; `parentOf`/`findHoleParent`/`findMirror` are early-exit target searches (`walkTemplate` has no short-circuit, so visitor-izing them means a full traversal to find one node); `hasDeepConstruct` has a depth-conditional predicate the generic visitor has no parameter slot for; `recordSites` threads a stateful `documentOrder` counter through its own pass-specific recursion. Closed as won't-do — decision recorded in `server/tsrx/LE_TRUC_COMPILER.md` §7 with the per-walk reasoning; `walk.ts`'s own doc comment already named the holdouts.
  **Check:** No code change; documentation-only decision record.

- [x] LT-047: Direct unit test for `evaluability.ts` (`dependenciesOf`/`isServerEvaluable`) — done ✓
  **Skill:** le-truc-dev
  **Summary:** New `server/tests/tsrx/evaluability.test.ts` (5 tests) pins both functions directly against real compiled fixtures (via `compileSource`, matching `compiler.test.ts`'s pattern) rather than only transitively: signal-read dependency extraction with `JS_GLOBALS` (`String`, `Math`) stripped, an all-globals expression yielding an empty dependency set, `isServerEvaluable` true when every free name is in scope, false when one is withheld (the DOM-is-truth omission case), and true trivially for an empty dependency set regardless of scope.
  **Check:** `bun test server/tests/tsrx/evaluability.test.ts` 5/5; `bunx tsc --noEmit` clean; no change to existing goldens.

- [x] LT-048: Standalone unit tests for `analysis/loops.ts` (`runLoops`) and `analysis/naming.ts` — done ✓
  **Skill:** le-truc-dev
  **Summary:** New `server/tests/tsrx/loops-naming.test.ts` (10 tests), same "hand-build an `AnalysisContext`, call one pass" granularity as `analysis.test.ts`. `runLoops`: one test compiles a server-data `@for` fixture and asserts the resulting `ForClientPlan` (`itemParam`, `collection` naming, empty `effects`) plus the registered query, and one compiles a reactive `createList` `@for` fixture and asserts the `ReconcilePlan` (`signal`, `itemParam`, `keyParam`, `tag`) while confirming Pass 1 skips it (`forPlans.size === 0`). `naming.ts`: `uniqueName` (free name passthrough, `base2`/`base3` collision suffixing, claiming); `addQuery` (register-and-return, dedup by selector+cardinality, cardinality-distinct entries, child-tag registration keyed off the selector's own leading tag token — not the component's — and self-tag exclusion).
  **Check:** `bun test server/tests/tsrx/loops-naming.test.ts` 10/10; `bunx tsc --noEmit` clean; no change to existing goldens.

- [x] LT-049: Delete unused `diagnostic.withLine` (`diagnostics.ts`) — done ✓
  **Skill:** le-truc-dev
  **Summary:** Confirmed via repo-wide grep (excluding `node_modules`) that `withLine` had exactly one occurrence — its own definition — across `server/` and `.tsrx` sources; deleted.
  **Check:** `grep -rn "withLine" server/` → no hits; `bunx tsc --noEmit` clean; `bun test server/tests` unchanged (1371/0, +15 from the new LT-047/LT-048 test files).
