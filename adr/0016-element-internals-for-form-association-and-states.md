...
- Amended by: [ADR 0020](0020-merge-based-validity-composition-and-relayvalidity.md) — fixes `setCustomValidity` (§5) to merge validity flags instead of replacing them, and adds `relayValidity()` as a second escape-hatch helper (§7) for relaying a wrapped native control's full `ValidityState`. §1–4, §6, §8 unchanged.
- Supersedes: None
