...
**`run()` — registering hand-authored descriptors:** `watch()`/`on()`/`pass()`/`each()`/`provideContexts()` cover the vast majority of effects, but some components need to wrap non-le-truc reactive primitives or native browser APIs (e.g. `IntersectionObserver`, `createEffect()` from `@zeix/cause-effect` composed with a native event listener) in a hand-authored `EffectDescriptor` — a raw `() => MaybeCleanup` thunk not produced by any helper. Before this addition, `return` was the *only* registration path for such a descriptor (`FactoryResult` has always permitted authoring one by hand), which meant components using this pattern couldn't drop explicit `return` even after adopting implicit collection everywhere else.

`FactoryContext` gains a fifth plain-verb helper: `run: (descriptor: EffectDescriptor) => void`. It does exactly what `watch()`/`on()`/`pass()`/`provideContexts()` already do internally — `pushDescriptor(host, 'run', descriptor)` — with no new collector mechanism. This ships in **v2.3** alongside the rest of implicit collection, not deferred to v3: it's purely additive, closes the only remaining gap in the migration path immediately, and means v3 has nothing new to design — it simply removes the deprecated `return` form, with `run()` already the established replacement for the one case `return` was still required for.
...
- **Keep `return` alive permanently as an escape hatch for hand-authored descriptors only, instead of adding `run()`.** Rejected — re-introduces the dual-path complexity ADR 0018's own v3.0 milestone (and the LT-005 `forEachUnseen()` consolidation) is meant to retire, and permanently blocks `FactoryResult`/`EffectDescriptor` from leaving the public return contract.
- **Point authors at `createEffect()` (from `@zeix/cause-effect`) directly instead of a hand-authored `EffectDescriptor` via `run()`.** Rejected — a factory-scope `createEffect()` call runs immediately, outside deferred activation, so it would violate M8 (no guarantee child custom elements are defined first) and wouldn't get automatic cleanup wired into the component's disconnect scope.
