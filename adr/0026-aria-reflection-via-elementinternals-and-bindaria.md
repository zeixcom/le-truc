...
Lift ADR 0016's advisory and amend its "ARIA reflection: available but not promoted" section. ARIA reflection via `ElementInternals` is a **recommended, first-class channel** alongside content attributes, governed by a two-channel policy, served by one new binding helper (`bindAria()`), and made tooling-visible by implementing the element-internals-declaration protocol for every Le Truc component. Additive and non-breaking; `bindAttribute`/`bindProperty` keep their roles.
