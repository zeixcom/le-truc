...
**[Amended 2026-09-19, owner, at LT-271: the code prefix is `LTC###`, except for the six `.tsrx`-grammar codes.** The codes in this ADR's prose and tables are spelled `TSRX###` because the compiler had one front end when it was written. [ADR 0034](0034-distribution-tsx-only-compiler-package-and-template-emission.md) s2 publishes the `.tsx` front end only at v3.0, so a code named after `.tsrx` would reach authors who cannot use that surface. The 44 surface-neutral codes are renamed to **`LTC###`** (`@zeix/le-truc-compiler`) keeping their numbers, so this ADR's rows re-point mechanically — `TSRX026` is `LTC026`, `TSRX012` is `LTC012`, and so on. The six codes that diagnose `.tsrx` grammar specifically — `TSRX018`, `TSRX020`, `TSRX021`–`TSRX024` — keep the `TSRX` prefix, because they name the surface correctly and are not in the published package. The one-directional rule in this sub-design is unchanged; read "a `TSRX` diagnostic" as "an `LTC` diagnostic". The spent-number ledger below survives the rename intact. Full disposition: [VOCABULARY_LEDGER.md](../server/compiler/VOCABULARY_LEDGER.md).]**
...
**Amended by [ADR 0029](0029-tiered-server-evaluation.md): "already correct" now varies by evaluation tier, and is weakest exactly where a degraded component is most likely to be noticed.** A Folded-tier component's pre-JS markup carries every reactive initial value, so the claim above holds unchanged; a Simulated-tier component's carries the realm's answers. But a **Static-tier** component's markup deliberately *omits* what no server phase could resolve — as does any unresolvable expression in any tier (ADR 0029 sub-design 1) — so its degraded state is not the intended state, it is the absent one. That is usually right, and it is the accepted trade: a scroll-overflow state genuinely has no server answer. It is not right for a semantically-loaded attribute on a real submittable form control, which is exactly why `TSRX034`'s severe variant survives instead of dissolving into the tier census — scoped per-expression, so it fires for any such site no server phase can resolve, on a Static-tier component or on a Simulated-tier one carrying that single unresolvable site. The mitigation is authored, not automatic: give such an element a static or server-rendered default so the no-JS state is the one the author chose.
...
| `NoActiveCollectorError` | `pushDescriptor` | **`TSRX045`** ✅ (LT-157d) for a collector helper deferred into a callback — **note: [ADR 0029](0029-tiered-server-evaluation.md) split `TSRX013` and this rule takes its own code, since it is a client-side bug rather than a server-evaluation guard; re-pointed from `TSRX013` when LT-165 landed [2026-09-04]** — plus **`TSRX008`** ✅ for an `async` component function. The compiler cannot emit the shape, so the rule exists entirely for authored setup | **Contained** |
