...
1. **A substantial set of components genuinely need something phase 1 cannot supply.** The count depends entirely on what is being counted, and only one predicate is the right one. Counting components that need a DOM fact for *any purpose* — every `first()`, every stubbed-API read, wherever it appears — leaves roughly 6 of 22 qualifying for the cheap path. But sub-design 1's first test counts something narrower: sites whose *server render* phase 1 cannot complete. A `first()` read confined to `watch()`/`on()` positions is a client concern (ADR [0003](0003-attributes-drive-state-at-connect-time-only.md)) and reaches no served byte, and most stubbed-API reads (`matchMedia`, layout geometry, `internals.states`) sit in the same client-only positions. Under the predicate that actually routes, the classifier folds **19 of 22** — the cheap path is the majority path, with `basic-pluralize`, `form-combobox` (via compose-read) and `form-listbox` Simulated, and none Static.

2. **A significant set of components cannot be answered by phase 2 either, and they are the expensive ones.** `module-scrollarea` — 2,091 occurrences, the single largest cost driver in the corpus — reads `host.scrollLeft`/`scrollTop`/`scrollWidth`/`offsetWidth`/`scrollHeight`/`offsetHeight` and emits *exclusively* through `bindState(internals, …)`. ADR 0027's own accepted tradeoffs make both of those unanswerable by design: jsdom layout reads return silent zeros, and sub-design 2 normalizes `attachInternals()` to throw so every component takes the library's graceful-degradation branch, which means internals-only output "renders as nothing in the served HTML — permanently." Simulating `module-scrollarea` therefore costs ~2.3 s of build time to compute nothing. It is not alone: `card-mediaqueries` (`matchMedia`), `form-colorgraph` (`getBoundingClientRect`), `form-textbox` and `form-spinbutton` (`internals.states`), and the unmigrated `module-carousel`/`ticker`/`splitview`/`dialog`/`section-menu` are all in the same position. These reads are unanswerable exactly as stated. Whether the component carrying them lands in the Static tier is a separate question: most of these sit in client-only positions — scroll and observer callbacks, `bindState` output channels — that never reach served HTML, so the components fold. The Static tier is not this list; it is the narrower case of an unanswerable expression in a *served* position with no realm-answerable sibling signal.
...
What the tier flag changes is one thing: **a Simulated-tier or Static-tier module drops the parts of the `@{ }` setup its own markup does not need.** The verbatim re-declaration exists so the value harness can compute signal initial values, and it is exactly the construct that `TSRX013` and `TSRX043` were invented to protect. A Simulated-tier module leaves what it drops to the realm; a Static-tier module leaves the same to the client. Only the Folded tier re-declares setup in full, and by construction a component that does so is one whose setup the harness can evaluate totally.

The skeleton and the value harness are **not separable layers**, so the flag cannot be implemented as "emit the skeleton, drop the setup". `lazyValueExpression` splices `<name>.get()` directly into the markup, which makes the folded markup partly the harness's own output: a folded signal is not dead code server-side, and dropping its declaration emits a module that references an undeclared name.

The flag therefore applies one criterion rather than a layer split: **retain a setup statement when the emitted markup depends on its declared name, transitively; drop the rest.** Three cases that would otherwise need special-casing fall out of it. A plain const is retained when the skeleton interpolates it — `form-combobox`'s `inputId` folds into `<label for>`, `<input id>`, `<p id>` and `aria-describedby`, and nothing downstream restores them. A folded signal is retained for the same reason. `expose()` is dropped without being named, because it declares nothing the markup can reference: an exposed-prop lazy child resolves through the prop→signal map at compile time to a literal. What the flag removes across the corpus is `expose()`, its `refStub` any-stubs, and the event-handler and helper consts.

The invariant that makes the flag safe is that **the emitted markup is byte-identical across all three tiers**, so routing a component downward never changes what a no-JS reader sees before the realm or the client gets involved.
...
| `TSRX034`, **severe** (`disabled`/`checked` on a real submittable control) | error | **Survives, scoped per-expression.** It fires iff the SITE's own resolution is `none` — unresolvable in every tier, so the value is omitted however the component routes, and its own copy is right that a submittable control rendering the wrong `disabled`/`checked` is a correctness bug, not a flash. A realm-answerable severe site stays silent, because the realm renders the value and the diagnostic would be noise. The scope is the expression, not the component: a `none` site on a component routed Simulated by some OTHER signal still omits its value, and a per-component Static check would have missed it. Sound without a tier check — a `none` resolution can never occur on a Folded-tier component, since the signal recording it would have made the component non-Folded. Re-grounded in the classifier's resolution rather than in the retired fold. |
...
**[Amended 2026-09-06, architect, at LT-165 step 8's landing: the byte-identity rule is structurally void, and the audit pins a recorded connect diff instead.** In a Folded-tier component every signal seeds from a DOM harvest — a signal with no harvestable render site is a `TSRX004` routing signal and routes Simulated — so the realm's entire state derives from the phase-1 bytes themselves: the client recomputes from what the server served, and the two mechanisms cannot independently disagree on a server-rendered value. Byte comparison therefore measures only the hydration boundary, and the first run's census found every divergence in three designed classes — serializer normalization (bare boolean attributes round-trip as `attr=""`, attribute case lowercases), the client's connect-time writes (ADR 0003's enhancement posture: fills of initially-empty sites, roving `tabindex`, selection state, jsdom-driven styles), and writes that overwrite or remove server-rendered state — the dangerous class, of which the census caught one instance (form-tokenbox's text input removed at hydration; LT-185). The audit (`equivalence-audit.test.ts`) now renders every Folded-tier component through the realm and pins the per-component connect diff as a snapshot: a diff that changes is a review trigger naming that component, not an automatic mechanism failure. The corpus cost figure above stands. Sub-design 4's byte-identity invariant is unaffected — it speaks to the three tiers' server modules, not to hydration.]
...
- the Folded tier is the majority path (19 of 22), so tiering's payoff comes mostly from what the Static tier avoids rather than from widening the Folded tier. What tiering buys is not simulating components whose unanswerable reads reach no served byte — under sub-design 1's predicate those fold totally, so the ~2.3 s of `module-scrollarea` is never paid, by a cheaper tier than the original framing forecast. The Static tier is real but rare by construction, and an empty Static census is a correct classification of the current corpus, not a wiring failure.
