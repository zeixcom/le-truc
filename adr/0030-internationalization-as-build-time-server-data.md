...
**Argument checking is a build-time diagnostic on both surfaces, and a type on `.tsx`.** The compiler validates a call site's arguments against the declared pattern's parsed argument set — a `TSRX` code, tier **Prevented** (statically decidable, author-fixable) — in the shared post-lowering pass, so it cannot drift between the two authored surfaces. `t` is typed **per key** (amended 2026-09-25, owner): the author declares `export const i18n = { … } as const` and annotates `i18n: I18n<typeof i18n>`. On the authored side, `I18n<M>` maps each literal value by shape: a pattern containing `{` is `(args: MessageArgs) => string`, anything else is `string`. A value widened to `string` (the `as const` forgotten) maps to the `string | ((args) => string)` union, so the omission fails tsc at the first attribute that reads the key instead of passing silently. The compiler rewrites the annotation in each generated module to the exact record its parse already knows: `string` for an argument-less message, `(args: { count: number }) => string` for one with arguments. The argument types follow the pattern (`plural`/`selectordinal`/`number` → `number`, `date`/`time` → `Date | number`, otherwise `string`). The loose `string | ((args) => string)` union ruled on 2026-09-19 is withdrawn before it shipped. With that union, every attribute position that reads a message (`placeholder={t.filter}`) fails tsc, because a function is not an attribute value, and the escape hatches (widening attributes, or casts) reopen the gap LT-308 exists to close. TypeScript then catches an undeclared key and an argument-less misuse on `.tsx` (tier 1 Prevented). The compiler diagnostic stays the channel for `.tsrx`, which no tsc program sees. A computed `t[dynamicKey]` stays rejected: which keys a component references must be statically decidable.
...
- `t.<key>` is `string` for an argument-less message and a function otherwise, and its TypeScript type says which, per key (amended 2026-09-25; the loose union and its "do not rely on the wider type" caveat are withdrawn). The cost is that the compiler owns a type rewrite of the authored annotation in both generated modules. Changing a message's argument set is therefore a type change at every call site, which is the point.
