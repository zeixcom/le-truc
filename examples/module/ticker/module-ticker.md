...
To support 10,000+ rows smoothly, the component uses block-level virtualization. `each()` discovers each `<tbody>` block and wires it to a `createSensor` wrapping its own `IntersectionObserver`; a `watch` effect on the sensor replaces off-screen rows with a single height-matched placeholder. This strips DOM nodes and tears down their `watch` effects automatically via `each()`, making signal updates for virtualized rows practically free.
