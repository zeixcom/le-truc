...
test.describe('test-each: each() helper', () => {
	...
	test('initially no item is active', async ({ page }) => {
		const items = page.locator('test-each li')
		for (let i = 0; i < 3; i++) {
			await expect(items.nth(i)).not.toHaveClass(/active/)
		}
	})

	test('nested watch — marks correct item active when selected changes', async ({
		page,
	}) => {
		const items = page.locator('test-each li')

		await page.evaluate(() => {
			const el = document.querySelector('test-each') as any
			el.selected = 1
		})
	...
	test('nested watch — switches active class when selected changes again', async ({
		page,
	}) => {
		const items = page.locator('test-each li')
	...
	test('per-element lifecycle — dynamically added item gets effects', async ({
		...
		// Add item3 — each() will re-watch, set up watch/on for item3,
		// and the watch effect immediately fires with selected=3
		await page.evaluate(() => {
			const ul = document.querySelector('test-each ul')!
		...
		const newItem = page.locator('test-each li[data-index="3"]')
