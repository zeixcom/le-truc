 ...
 *
 * `server/compiler/frontend/tsrx/index.ts` is the same pure string→string pipeline
 ...
 * `buildTsrxBrowserBundle` is the reusable half: `server/tests/compiler/
...
const ENTRY = resolve(ROOT, 'server/compiler/frontend/tsrx/index.ts')
...
export const assertNodeFree = (bundleText: string): void => {
	...
	if (hit)
		throw new Error(
			`Browser bundle of server/compiler/frontend/tsrx/index.ts contains a Node import (${hit[0]}…) — the compiler must stay browser-pure (ADR 0025 sub-design 6, LT-044/LT-045). Move the offending import behind a Node-only entry point (e.g. a CLI script under scripts/), not server/compiler/.`,
		)
}

/**
 * Builds `server/compiler/frontend/tsrx/index.ts` for the browser and returns its bundled
...
export const buildTsrxBrowserBundle = async (): Promise<string> => {
	...
	if (!result.success) {
		...
		throw new Error(
			`Browser bundle of server/compiler/frontend/tsrx/index.ts failed:\n${messages}`,
		)
	}
