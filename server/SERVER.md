...
The inlined TSRX compiler (ADR 0024) compiles isomorphic single-file `.tsrx` components — server args, signals, `expose()`, markup, event handlers, and scoped styles in one source — into the split compiler's two halves. The server module re-declares the `@{ }` setup against the runtime harness (`server/tsrx/runtime.ts`) and renders HTML strings; the client module is a generated factory importing solely from `@zeix/le-truc`. Extension activation is declared as `export const config` in the source; the compiler validates it, auto-imports the extension factories, and carries `expose()`/`defineMethod()` as ambients. `bun run build:cem` runs `scripts/build-tsrx.ts` before `cem analyze` so `@zeix/cem-plugin-le-truc` reads the generated clients unchanged. Errors fail the build; `@for` over a non-List reactive source logs `TSRX001` and skips the file.
...
The compiler lives in `server/tsrx/`, one concern per file, wired together by `compiler.ts` (`compileSource`). `core.ts` is the only module importing `@tsrx/core` values; `ir.ts` holds the shared IR types; `lower-template.ts` lowers JSX/`@if`/`@switch`/`@try`/`@for` into IR; `classify-attributes.ts`, `infer-type.ts`, `config.ts`, and `imports.ts` handle attributes, signal typing, `export const config`, and import resolution; `analysis/{plan,selectors,naming,harvest,loops,effects}.ts` produce the client emission plan; `emit-server.ts`/`emit-client.ts` render the artifacts. See `server/tsrx/LE_TRUC_COMPILER.md` for the full module inventory. Golden tests in `server/tests/tsrx/` pin server renders, CSS bytes, client snapshots (regenerate with `UPDATE_SNAPSHOTS=1 bun test server/tests/tsrx`), diagnostics, and language-feature gates.

#### Server Simulation driver (`server/tsrx/sim/`, ADR 0027)

Renders initial HTML by *executing* the generated client module against jsdom instead of evaluating it syntactically. The substrate layer is portable; the build calls it through the simulation pass below.

- **`patch-table.ts`** — declarative patch data: `REALM_GLOBALS` forces DOM constructors from the jsdom window onto `globalThis` (some runtimes ship natives that would otherwise shadow the realm and break `instanceof`); `STUB_GLOBALS` gives absent constructors inert no-ops; `NETWORK_GLOBALS` replaces `fetch`/`XMLHttpRequest`/`WebSocket`/`EventSource`/`Request`/`navigator.sendBeacon` with rejecting no-ops so a build can never depend on the network; `PROTOTYPE_PATCHES` is empty since LT-177 — `attachInternals()` is deliberately left alone, so jsdom's skeletal `ElementInternals` reaches the library and `bindAria()` can bind the host content attribute that the served HTML carries.
