...
#### Server Simulation driver (`server/tsrx/sim/`, ADR 0027 — not yet wired into the build)

Groundwork for rendering initial HTML by *executing* the generated client module against jsdom instead of evaluating it syntactically. LT-151 delivers the portable substrate layer only; the build pipeline does not call it yet (LT-154).

- **`patch-table.ts`** — declarative data, one list per concern. `REALM_GLOBALS` are forced onto `globalThis` from the jsdom window (forcing, not filling: Deno ships native `Event`/`CustomEvent` that would otherwise *shadow* the realm and silently break `instanceof`); `STUB_GLOBALS` gives absent constructors inert no-ops (`ResizeObserver`, `matchMedia`, `IntersectionObserver`, and a forced `requestAnimationFrame` whose callback could never reach the HTML anyway); `NETWORK_GLOBALS` replaces `fetch`/`XMLHttpRequest`/`WebSocket`/`EventSource`/`Request`/`navigator.sendBeacon` with reporting, rejecting no-ops so a build can never depend on the network (sub-design 2d); `PROTOTYPE_PATCHES` normalizes `attachInternals()` to throw so every substrate takes the same graceful-degradation branch (LT-150).
- **`realm.ts`** — the uniform applier. Two phases, because a client module registers its element as an *import side effect* and importing is asynchronous: `load()` imports with a **recording `customElements`** that captures `define()` calls, and `render()` then parses the SSR'd markup (elements still undefined — the pre-parsed upgrade path), replays the definitions so the upgrade runs, and serializes. Every touched global is restored by `dispose()`. jsdom's `virtualConsole`, attempted network calls, unhandled rejections and contained throws all land in `diagnostics`, attributed to the component whose window was open.
- **`boundary.ts`** — `runSynchronously()`, the driver-level assertion that the instantiate→serialize window never awaited (ADR 0024 s13's *guarantee*, ADR 0027 sub-design 9). A build-time data dependency awaits in the resolution phase, before the simulation; the second phase is synchronous, so the compiler — not microtask timing — decides which `@try` arm ships.

`bun run check:sim` runs `scripts/sim-portability-check.ts`: it renders `form-colorgraph` (the ADR's stress case), bundles its client module, and runs the runtime-neutral `scripts/sim-portability-probe.ts` under every runtime on PATH, diffing the serialized HTML. It exits non-zero when they disagree.
