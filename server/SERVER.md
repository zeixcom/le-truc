...
- **`server/compiler/simulation/resolve.ts`** — activation is installation. The driver specifier is held in a variable so the typechecker does not follow it; absence answers `null` and a version mismatch throws.
