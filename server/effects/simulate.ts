...
export type SimulationPassResult = {
	...
	/** `render()` calls — one per occurrence. */
	occurrences: number
...
export const simulateTsrxCorpus = async ({
	...
	try {
		...
		for (const subject of subjects) {
			...
			for (const occurrence of occurrencesOf(realm, subject.tag, markup)) {
				await realm.render({ markup: occurrence, component: subject.tag })
				occurrences++
			}
		...
		log(
			`🎭 Simulation pass: ${simulated.length} Simulated-tier component(s), ` +
				`${occurrences} occurrence(s), ${renders} render(s), ` +
