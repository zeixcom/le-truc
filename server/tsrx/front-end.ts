 ...
 * `.tsrx` (`compiler.ts`) and `.tsx` (`server/tsrx-tsx/compiler-tsx.ts`) —
 ...
 * args object (TSRX008). Pushes the diagnostic and returns null otherwise.
 * A destructured default paired with a non-optional type is TSRX032
 * (CHECKLIST §10): the type annotation is what callers see, and it says the
 * prop is required, so the default is unreachable for any external caller.
 */
