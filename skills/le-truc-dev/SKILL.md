...
**`asParser()` and `asMethod()` branding is required.** Detection falls back to `fn.length >= 2` for parsers — unreliable with default params or destructuring. Always brand custom parsers and method producers.
