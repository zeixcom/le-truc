 ...
 * was not called synchronously during factory setup, an `each()` callback, or a
 * `reconcile()` `bindItem` (for example: after an `await`, inside a detached
 * `setTimeout`, or from an event handler defined during setup).
 *
 * @since 2.3
